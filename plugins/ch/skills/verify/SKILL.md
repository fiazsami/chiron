...
5. **Material behind its source.** `./ch doc --status` for any register whose
   `translation.yaml` names `material: derived/<recipe>`. This is the one kind
   of drift the state model cannot see: `content_hash` is computed over the
   generated bytes, so when the *source* moves and the docs have not been
   regenerated, every chapter still reads `ok` while the material silently
   describes an older commit. `--status` exits 1 when any tree is behind.
   Report it as its own line — never fold it into the dimension states, which
   are telling the truth about a different question.

The read-only rule covers `./ch doc` too: `--status` and `--census` write
nothing, and are the only two forms this skill runs. `--stage` and `--promote`
belong to `/ch:mount`.
...
4. One line per derived tree that is behind its source, naming the commit it
   was generated from and the commit the source is at now.
5. Anything needing a human decision, **stated as a decision, not a task** —
