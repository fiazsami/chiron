...
# Shown to an authoring agent when the chapter it is reading was produced by a
# documentation generator rather than written. Without it the lexicon fills
# with the generator's boilerplate, which is grammatically fine, verifiably
# anchored, and completely worthless as working language.
GENERATED_MATERIAL_NOTE = """\
This chapter was generated from the corpus's source code by a documentation
generator. Two kinds of text are in front of you, and only one is the
corpus's working language:

- REAL — identifiers exactly as the authors spelled them (types, functions,
  parameters, modules), signatures, and the prose of the authors' own doc
  comments. This is what you are here for.
- SCAFFOLDING — sentences the generator wrote because it had to write
  something: "Returns the value of x", "Inherited from Base", "Objects",
  parameter tables restating a signature, navigation stubs. It is not
  vocabulary, and no one instructing an agent would ever say it.

Anchor on the first kind. A term whose only evidence is scaffolding does not
belong in the lexicon, however quotable the scaffolding is."""
...
def requirements_lines(corpus: Corpus) -> list[str]:
    ...
    if corpus.generated_by:
        out.append(f"- material: GENERATED by {corpus.generated_by}")
    out.append("")
    if corpus.generated_by:
        out.append(_block("generated-material", GENERATED_MATERIAL_NOTE))
